                ...
                for file in os.listdir(folder_path):
                    if file.endswith(".json"):
                        ...
                        def delete_block(block_name, folder_path):
                            ...
                            png_path = os.path.join(mod_folder, f"sprites/{block_type}/{block_name}/{block_name}.png")
                            block_folder_path = os.path.join(mod_folder, f"content/blocks/{block_type}")
                            ...
                            try:
                                ...
                                for path in [block_folder_path, sprite_folder_path]:
                                    if os.path.isdir(path):
                                        try:
                                            # Если папка пуста — удалим
                                            if not os.listdir(path):
                                                os.rmdir(path)
                                                print(f"Папка {path} удалена (пустая).")
                                        except FileNotFoundError:
                                            pass  # Папка уже была удалена
                        ...
                        tk.Button(
                            frame,
                            text="Удалить",
                            command=lambda b=block_name: delete_block(b, folder_path)
                        ).pack(side="right")
