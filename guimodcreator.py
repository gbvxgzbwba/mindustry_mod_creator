    ...
    def main():   
        ...
        def create_block():
            ...
            def editor_cb():
                ...
                tk.Button(root, text="⬅️ Назад", font=0, command=lambda: create_block()).pack(pady=10)

                tk.Label(root, text="Выберите категорию блоков:", font=("Arial", 12)).pack()

                blocks_path = os.path.join(mod_folder, "content", "blocks")

                # Показать все папки в content/blocks
                for folder_name in os.listdir(blocks_path):
                    folder_path = os.path.join(blocks_path, folder_name)
                    if os.path.isdir(folder_path):
                        tk.Button(
                            root,
                            text=folder_name,
                            command=lambda f=folder_path: open_block_folder(f)
                        ).pack(pady=2)
            ...
            def cb_wall_create():
                ...
                def save_wall():
                    ...
                    block_data = {
                        ...
                        "health": health,
                        "size": size,
                        "_comment": "name это имя в игре, description это описания в игре, health это количество здоровья, size это размер, requirements это ресурсы для постройки",
                        "category": "defense",
                        "type": "wall",
                        "requirements": [],
                        "research": { 
