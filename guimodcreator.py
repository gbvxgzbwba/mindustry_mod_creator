    ...
    def main():   
        ...
        def create_block():
            ...
            def cb_ore_create():
                ...
                tk.Label(root, text="Создание руды", font=("Arial", 14)).pack(pady=10)

                # Имя руды
                tk.Label(root, text="Имя руды:").pack()
                entry_name = tk.Entry(root)
                entry_name.pack()

                # Описание предмета
                tk.Label(root, text="Описание предмета:").pack()
                entry_description = tk.Entry(root)
                entry_description.pack()

                # Твёрдость (hardness) через Spinbox
                tk.Label(root, text="Твёрдость (1-15):").pack()
                spin_hardness = tk.Spinbox(root, from_=1, to=15, width=5)
                spin_hardness.pack()

                def create_ore_files():
                    name = entry_name.get().strip()
                    description = entry_description.get().strip()
                    try:
                        hardness = float(spin_hardness.get())
                        if hardness < 1 or hardness > 15:
                            raise ValueError
                    except ValueError:
                        messagebox.showerror("Ошибка", "Введите корректную твёрдость от 1 до 15!")
                        return

                    # Пути к папкам
                    ores_folder = os.path.join(mod_folder, "content", "blocks", "environment")   # исправил на ores
                    items_folder = os.path.join(mod_folder, "content", "items", "ore")
                    sprite_folder = os.path.join(mod_folder, "sprites", "ore", name)

                    os.makedirs(ores_folder, exist_ok=True)
                    os.makedirs(items_folder, exist_ok=True)
                    os.makedirs(sprite_folder, exist_ok=True)

                    # Создание JSON для блока руды
                    ore_block_data = {
                        "type": "OreBlock",
                        "itemDrop": name
                    }
                    ore_block_path = os.path.join(ores_folder, f"{name}.json")
                    with open(ore_block_path, "w", encoding="utf-8") as f:
                        json.dump(ore_block_data, f, indent=4, ensure_ascii=False)

                    # Создание JSON для предмета
                    ore_item_data = {
                        "name": name,
                        "description": description,
                        "hardness": hardness,
                    ...
                    ore_item_path = os.path.join(items_folder, f"{name}.json")
                    with open(ore_item_path, "w", encoding="utf-8") as f:
                        json.dump(ore_item_data, f, indent=4, ensure_ascii=False)

                    # Текстуры
                    base_texture_url = "https://raw.githubusercontent.com/gbvxgzbwba/texture123/main/ore/"
                    default_textures = ["ore", "ore1", "ore2", "ore3"]

                    try:
                        for default_name in default_textures:
                            new_texture_name = default_name.replace("ore", name) + ".png"
                            texture_url = base_texture_url + default_name + ".png"
                            texture_path = os.path.join(sprite_folder, new_texture_name)

                            if not os.path.exists(texture_path):
                                try:
                                    urllib.request.urlretrieve(texture_url, texture_path)
                                    print(f"✅ Загружено: {new_texture_name}")
                                except Exception as e:
                                    print(f"❌ Ошибка загрузки {new_texture_name}: {e}")

                    except Exception as e:
                        print(f"❌ Ошибка при скачивании текстур: {e}")

                    messagebox.showinfo("Успех", f"Руда '{name}' и её предмет успешно созданы!")
                    создание_кнопки()

                tk.Button(root, text="Создать руду", command=create_ore_files, font=10).pack(pady=20)
