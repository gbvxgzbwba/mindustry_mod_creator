...
def load_or_create_cache():
    ...
    try:
        ...
        with open(path, "w", encoding="utf-8") as f:
            json.dump(default_cache, f, indent=4, ensure_ascii=False)
        return default_cache
CACHE_FILE = load_or_create_cache()
...
class MindustryModCreator:
    def main():   
        ...
        def open_mod_editor(load_existing=False):
            ...
            label_name = tk.Label(root, text="Название мода")
            entry_name_1 = tk.Entry(root, bg="#CFDDF3", width=50)
            label_author = tk.Label(root, text="Автор")
            entry_name_2 = tk.Entry(root, bg="#CFDDF3", width=50)
            label_description = tk.Label(root, text="Описание")
            entry_name_3 = tk.Entry(root, bg="#CFDDF3", width=50)

            # --- Добавляем выбор версии ---
            label_version = tk.Label(root, text="Минимальная версия игры")
            combo_version = ttk.Combobox(root, values=["146", "147.1"], state="readonly", width=10)
            combo_version.set("146")  # Значение по умолчанию

            if load_existing:
                mod_json_path = os.path.join(mod_folder, "mod.json")
                with open(mod_json_path, "r", encoding="utf-8") as file:
                    mod_data = json.load(file)
                entry_name_1.insert(0, mod_data.get("name", ""))
                entry_name_2.insert(0, mod_data.get("author", ""))
                entry_name_3.insert(0, mod_data.get("description", ""))
                combo_version.set(str(mod_data.get("minGameVersion", "146")))

            button_create = tk.Button(root, text="Создать mod.json", font=10, 
                                      command=lambda: create_mod_json(entry_name_1, entry_name_2, entry_name_3, combo_version))

            label_name.pack(pady=(30, 0))
            entry_name_1.pack(pady=(5, 0))
            label_author.pack(pady=(10, 0))
            entry_name_2.pack(pady=(5, 0))
            label_description.pack(pady=(10, 0))
            entry_name_3.pack(pady=(5, 0))
            label_version.pack(pady=(10, 0))
            combo_version.pack(pady=(5, 0))
            button_create.pack(side=tk.BOTTOM, pady=30)
        ...
        def создание_кнопки():
            ...
            label = tk.Label(root, text="Что добавить?", font=("Arial", 14))
            button_create_item = tk.Button(root, text="Создать предмет", font=10, command=create_item_window)
            button_create_liquid = tk.Button(root, text="Создать жидкость", font=10, command=create_liquid_window)
            button_create_block = tk.Button(root, text="Создать блок", font=10, command=create_block)

            label.pack(pady=(50, 20))
            button_create_item.pack(pady=(10, 0))
            button_create_liquid.pack(pady=(10, 0))
            button_create_block.pack(pady=(10, 0))

        def create_item_window():
            """Форма для создания предмета"""
            clear_window()

            label_name = tk.Label(root, text="Название предмета")
            entry_name = tk.Entry(root, width=50)

            label_desc = tk.Label(root, text="Описание")
            entry_desc = tk.Entry(root, width=50)

            label_flammability = tk.Label(root, text="Воспламеняемость (0-1)")
            entry_flammability = tk.Entry(root, width=10)

            label_explosiveness = tk.Label(root, text="Взрывоопасность (0-1)")
            entry_explosiveness = tk.Entry(root, width=10)

            label_radioactivity = tk.Label(root, text="Радиоактивность (0-1)")
            entry_radioactivity = tk.Entry(root, width=10)

            label_charge = tk.Label(root, text="Заряд (0-1)")
            entry_charge = tk.Entry(root, width=10)

            button_save = tk.Button(root, text="Сохранить предмет", font=10, 
                                    command=lambda: save_item(entry_name, entry_desc, entry_flammability, 
                                                              entry_explosiveness, entry_radioactivity, 
                                                              entry_charge))

            label_name.pack(pady=(10, 0))
            entry_name.pack(pady=(5, 0))
            label_desc.pack(pady=(10, 0))
            entry_desc.pack(pady=(5, 0))
            label_flammability.pack(pady=(10, 0))
            entry_flammability.pack(pady=(5, 0))
            label_explosiveness.pack(pady=(10, 0))
            entry_explosiveness.pack(pady=(5, 0))
            label_radioactivity.pack(pady=(10, 0))
            entry_radioactivity.pack(pady=(5, 0))
            label_charge.pack(pady=(10, 0))
            entry_charge.pack(pady=(5, 0))
            button_save.pack(pady=(20, 0))

        def save_item(entry_name, entry_desc, entry_flammability, entry_explosiveness, entry_radioactivity, entry_charge):
            """Сохраняет предмет в JSON"""
            name = entry_name.get().strip().replace(" ", "_")
            desc = entry_desc.get().strip()
            try:
                flammability = float(entry_flammability.get())
                explosiveness = float(entry_explosiveness.get())
                radioactivity = float(entry_radioactivity.get())
            ...
            if save_item:
                texture_url = f"https://raw.githubusercontent.com/gbvxgzbwba/texture123/main/ore/ore.png"  # Замените на реальный URL
            ...
            messagebox.showinfo("Успех", f"Предмет '{name}' сохранён в {item_file_path}!")
            создание_кнопки()
        ...
        def create_liquid_window():
            ...
            label_name = tk.Label(root, text="Название жидкости")
            entry_name = tk.Entry(root, width=50)

            label_desc = tk.Label(root, text="Описание")
            entry_desc = tk.Entry(root, width=50)

            label_viscosity = tk.Label(root, text="Густота (0-1)")
            entry_viscosity = tk.Entry(root, width=10)

            label_temperature = tk.Label(root, text="Температура (0-1)")
            ...
            label_color = tk.Label(root, text="Цвет (#ff0000)")
            entry_color = tk.Entry(root, width=10)

            button_save = tk.Button(root, text="Сохранить жидкость", font=10, 
                                    command=lambda: save_liquid(entry_name, entry_desc, entry_viscosity, 
                                                               entry_temperature, entry_flammability, 
                                                               entry_explosiveness, entry_color))

            label_name.pack(pady=(10, 0))
            entry_name.pack(pady=(5, 0))
            label_desc.pack(pady=(10, 0))
            entry_desc.pack(pady=(5, 0))
            label_viscosity.pack(pady=(10, 0))
            entry_viscosity.pack(pady=(5, 0))
            label_temperature.pack(pady=(10, 0))
            entry_temperature.pack(pady=(5, 0))
            label_flammability.pack(pady=(10, 0))
            entry_flammability.pack(pady=(5, 0))
            label_explosiveness.pack(pady=(10, 0))
            entry_explosiveness.pack(pady=(5, 0))
            label_color.pack(pady=(10, 0))
            entry_color.pack(pady=(5, 0))
            button_save.pack(pady=(20, 0))

        def save_liquid(entry_name, entry_desc, entry_viscosity, entry_temperature, entry_flammability, 
                        entry_explosiveness, entry_color):
            """Сохраняет жидкость в JSON"""
            liquid_name = entry_name.get().strip().replace(" ", "_")
            ...
            if not liquid_name or not description:
                messagebox.showerror("Ошибка", "Все поля должны быть заполнены!")
                return
            ...
            liquid_file_path = os.path.join(content_folder, f"{liquid_name}.json")
            with open(liquid_file_path, "w", encoding="utf-8") as file:
                json.dump(liquid_data, file, indent=4, ensure_ascii=False)

            messagebox.showinfo("Успех", f"Жидкость '{liquid_name}' сохранена в {liquid_file_path}!")
            создание_кнопки()
        ...
        def create_block():
            ...
            button_save_wall = tk.Button(root, text="Создать стену", font=0, command=lambda: cb_wall_create())
            button_save_wall.pack()
            button_save_solarpanel = tk.Button(root, text="Создать солнечную панель", font=0, command=lambda: cb_solarpanel_create())
            button_save_solarpanel.pack()
            button_save_ConsumeGenerator = tk.Button(root, text="Создать генератор", font=0, command=lambda: cb_ConsumesGenerator_create())
            button_save_ConsumeGenerator.pack()
            button_save_StorageBlock = tk.Button(root, text="Создать Хранилище", font=0, command=lambda: cb_StorageBlock_create())
            button_save_StorageBlock.pack()
            button_save_GenericCrafter = tk.Button(root, text="Создать Завод", font=0, command=lambda: cb_GenericCrafter_create())
            button_save_GenericCrafter.pack()
            button_save_conveyor = tk.Button(root, text="Создать конвеер", font=0, command=lambda: cb_conveyor_create())
            button_save_conveyor.pack()
            button_save_conduit = tk.Button(root, text="Создать трубу", font=0, command=lambda: cb_conduit_create())
            button_save_conduit.pack()
            button_save_bridge_conveyor = tk.Button(root, text="Создать предметный мост", font=0, command=lambda: cb_bridge_conveyor_create())
            button_save_bridge_conveyor.pack()
            button_save_bridge_conveyor_energy = tk.Button(root, text="Создать предметный мост (работает на энергии)", font=0, command=lambda: cb_bridge_conveyor_energy_create())
            button_save_bridge_conveyor_energy.pack()
            button_save_bridge_liquid = tk.Button(root, text="Создать жидкосный мост", font=0, command=lambda: cb_bridge_liquid_create())
            button_save_bridge_liquid.pack()
            button_save_bridge_liquid_energy = tk.Button(root, text="Создать жидкосный мост (работает на энергии)", font=0, command=lambda: cb_bridge_liquid_energy_create())
            button_save_bridge_liquid_energy.pack()
            button_save_ore = tk.Button(root, text="Создать руду", font=0, command=lambda: cb_ore_create())
            button_save_ore.pack()
            button_back = tk.Button(root, text="Назад", font=0, command=lambda: создание_кнопки())
            button_back.pack()
            ...
            def cb_wall_create():
                ...
                # ✅ Чтение cache.json
                with open(resource_path("cache.json"), "r", encoding="utf-8") as f:
                ...
                def save_wall():
                    ...
                    with open(resource_path("cache.json"), "w", encoding="utf-8") as f:
                ...
                tk.Button(root, text="Сохранить", command=save_wall).pack(pady=20)
            ...
            def cb_solarpanel_create():
                ...
                research_parent_entry = ttk.Combobox(
                    root,
                    values=CACHE_FILE.get("solar_panel", []),
                    state="readonly",
                    width=30
                )
                research_parent_entry.pack()
                ...
                def save_panel():
                    ...
                    if name not in CACHE_FILE.get("solar_panel", []):
                        CACHE_FILE["solar_panel"].append(name)

                    with open(resource_path("cache.json"), "w", encoding="utf-8") as f:
                        json.dump(CACHE_FILE, f, indent=4, ensure_ascii=False)  # ✅

                    if name_exists_in_content(mod_folder, name, "solar_panel"):
                        return  # Остановить сохранение

                    # откроем новое окно для добавления ресурсов
                    open_requirements_editor(name, block_data)

                tk.Button(root, text="Сохранить", command=save_panel).pack(pady=20)
            ...
            def cb_ConsumesGenerator_create():
                ...
                research_parent_entry = ttk.Combobox(
                    root,
                    values=CACHE_FILE.get("Consumes_generator", []),
                    state="readonly",
                    width=30
                )
                research_parent_entry.pack()
                ...
                def save_ConsumesGenerator():
                    ...
                    if name not in CACHE_FILE.get("Consumes_generator", []):
                        CACHE_FILE["Consumes_generator"].append(name)

                    with open(resource_path("cache.json"), "w", encoding="utf-8") as f:
                        json.dump(CACHE_FILE, f, indent=4, ensure_ascii=False)  # ✅

                    if name_exists_in_content(mod_folder, name, "Consumes_generator"):
                        return  # Остановить сохранение

                    # откроем новое окно для добавления ресурсов
                    open_item_consumes_editor(name, block_data)

                tk.Button(root, text="Сохранить", command=save_ConsumesGenerator).pack(pady=20)
            ...
            def cb_StorageBlock_create():
                ...
                research_parent_entry = ttk.Combobox(
                    root,
                    values=CACHE_FILE.get("Storage_block", []),
                    state="readonly",
                    width=30
                )
                research_parent_entry.pack()
                ...
                def save_StorageBlock():
                    ...
                    if name not in CACHE_FILE.get("Storage_block", []):
                        CACHE_FILE["Storage_block"].append(name)

                    with open(resource_path("cache.json"), "w", encoding="utf-8") as f:
                        json.dump(CACHE_FILE, f, indent=4, ensure_ascii=False)  # ✅

                    if name_exists_in_content(mod_folder, name, "Storage_block"):
                        return  # Остановить сохранение

                    open_requirements_editor(name, StorageBlock_data)

                tk.Button(root, text="Сохранить", command=save_StorageBlock).pack(pady=20)
            ...
            def cb_GenericCrafter_create():
                ...
                tk.Label(root, text="Введите скорость производства 1 предмета в 1 секунду (1= 1 предмет в секунду, 2= 1 предмет в 2 секунды-(если он 1))").pack()
                ...
                research_parent_entry = ttk.Combobox(
                    root,
                    values=CACHE_FILE.get("Generic_crafter", []),
                    state="readonly",
                    width=30
                )
                research_parent_entry.pack()
                ...
                def save_GenericCrafter():
                    ...
                    if name not in CACHE_FILE.get("Generic_crafter", []):
                        CACHE_FILE["Generic_crafter"].append(name)

                    with open(resource_path("cache.json"), "w", encoding="utf-8") as f:
                        json.dump(CACHE_FILE, f, indent=4, ensure_ascii=False)  # ✅

                    if name_exists_in_content(mod_folder, name, "Generic_crafter"):
                        return  # Остановить сохранение

                    # откроем новое окно для добавления ресурсов
                    open_item_GenericCrafter_editor(name, block_data)

                tk.Button(root, text="Сохранить", command=save_GenericCrafter).pack(pady=20)
            ...
            def cb_conveyor_create():
                ...
                tk.Button(root, text="Сохранить", command=save_conveyor).pack(pady=20)
            ...
            def cb_conduit_create():
                ...
                tk.Button(root, text="Сохранить", command=save_conduit).pack(pady=20)
            ...
            def cb_bridge_conveyor_create():
                ...
                tk.Button(root, text="Сохранить", command=save_bridge_conveyor).pack(pady=20)
            ...
            def cb_bridge_liquid_create():
                ...
                tk.Button(root, text="Сохранить", command=save_bridge_liquid).pack(pady=20)
            ...
            def cb_ore_create():
                ...
                if os.path.exists(mod_items_path):
                    for f in os.listdir(mod_items_path):
                        if f.endswith(".json"):
                            ...
                            # Проверка, существует ли уже руда с этим именем
                            ore_path = os.path.join(ores_folder, f"{item_name}.json")
                            if not os.path.exists(ore_path):  # Если руда не существует
                ...
                def show_item_info(name):
                    ...
                    if os.path.exists(path):
                        ...
                        messagebox.showerror("Ошибка", f"Файл предмета '{name}' не найден.")

                def show_ore_info(name):
                    if not name:
                    ...
                    if os.path.exists(path):
                        ...
                        messagebox.showerror("Ошибка", f"Файл руды '{name}' не найден.")
                ...
                def add_ore():
                    ...
                    # Проверка, если руда с таким именем уже существует
                    ore_path = os.path.join(ores_folder, f"{name}.json")
                    if os.path.exists(ore_path):
                        messagebox.showerror("Ошибка", f"Руда с именем '{name}' уже существует!")
                        return
                    ...
                    try:
                        ...
                        messagebox.showerror("Ошибка", "Введите корректное значение твёрдости!")
                        return
                ...
                def change_hardness():
                    selection = ores_listbox.curselection()
                    if not selection:
                        messagebox.showerror("Ошибка", "Выберите руду для изменения твёрдости!")
                        return

                    ore_name = ores_listbox.get(selection[0]).split(" — ")[0]
                    # Ввод новой твёрдости
                    try:
                        new_hardness = simpledialog.askinteger("Изменение твёрдости", f"Введите новую твёрдость для руды '{ore_name}' (1-15):", minvalue=1, maxvalue=15)
                        if new_hardness is None:
                            return
                    except ValueError:
                        messagebox.showerror("Ошибка", "Введите корректное значение твёрдости!")
                        return

                    ore_path = os.path.join(ores_folder, f"{ore_name}.json")
                    if os.path.exists(ore_path):
                        with open(ore_path, "r+", encoding="utf-8") as f:
                            data = json.load(f)
                            data["hardness"] = new_hardness
                            f.seek(0)
                            json.dump(data, f, indent=4, ensure_ascii=False)
                            f.truncate()

                        # Обновление списка руд
                        for i in range(ores_listbox.size()):
                            if ores_listbox.get(i).startswith(ore_name):
                                ores_listbox.delete(i)
                                ores_listbox.insert(i, f"{ore_name} — Твёрдость: {new_hardness}")
                                break
                    else:
                        messagebox.showerror("Ошибка", f"Руда '{ore_name}' не найдена!")
                ...
                tk.Button(button_frame, text="Изменить твёрдость", command=change_hardness).grid(row=1, column=0, columnspan=2, pady=5)

                info_frame = tk.Frame(root)
                info_frame.pack(pady=5)

                tk.Button(info_frame, text="Инфо о предмете (?)", command=lambda: show_item_info(items_listbox.get(items_listbox.curselection()) if items_listbox.curselection() else None)).grid(row=0, column=0, padx=5)
                tk.Button(info_frame, text="Инфо о руде (?)", command=lambda: show_ore_info(ores_listbox.get(ores_listbox.curselection()).split(" — ")[0] if ores_listbox.curselection() else None)).grid(row=0, column=1, padx=5)
